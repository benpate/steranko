...
func (s *Steranko[T]) SigninFormPost(ctx echo.Context, session data.Session) (User, error) {
	...
	if err := s.authenticate(session, txn.Username, txn.Password, user); err != nil {
	...
	if err := s.SigninUser(ctx, session, user); err != nil {
...
func (s *Steranko[T]) SigninUser(ctx echo.Context, session data.Session, user User) error {
	...
	if err := s.SetCookie(ctx, session, claims); err != nil {
...
func (s *Steranko[T]) SetCookie(ctx echo.Context, session data.Session, claims T) error {

	const location = "steranko.SetCookieFromClaims"

	// Create the JWT Token
	token, err := s.CreateJWT(session, claims)
...
func (s *Steranko[T]) CreateJWT(session data.Session, claims T) (string, error) {
	...
	keyID, key, err := s.keyService.GetCurrentKey(session)
...
func (s *Steranko[T]) ValidatePassword(plaintext string) error {
