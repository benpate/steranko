...
func (s *Steranko[T]) PostPasswordUpdate(ctx echo.Context, session data.Session) error {
	...
	if err := s.authenticate(session, txn.Username, txn.OldPassword, user); err != nil {
	...
	if err := s.userService.Save(session, user, "Steranko: User Requested Password Update"); err != nil {
